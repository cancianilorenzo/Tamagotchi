...
void LCD_Initialization(void)
{
	...
	LCD_Configuration();
	delay_ms(100);
	DeviceCode = LCD_ReadReg(0x0000); /* ?????ID	*/
...
void LCD_DrawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
	...
	LCD_MyDrawLine(x0, y0, x2, y2, color);
	LCD_MyDrawLine(x1, y1, x2, y2, color);
}
/*
Function to draw a rectangle
*/
void draw_rectangle(double centerX, double centerY, double height, double length, int color){
double x0, y0, x1, y1;
x0 = centerX - (length/2);
x1 = centerX + (length/2);
y0 = centerY + (height/2);
...
*/
void draw_rectangle_full(double centerX, double centerY, double height, double length, int color){
double x0, y0, x1, y1;
x0 = centerX - (length/2);
x1 = centerX + (length/2);
y0 = centerY + (height/2);
